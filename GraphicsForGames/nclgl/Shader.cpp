...
Shader::Shader(string vFile, string fFile, string gFile, string cFile, string eFile) {
	...
	if (!cFile.empty()) {
		objects[SHADER_TESSCONTROL] = GenerateShader(cFile, GL_TESS_CONTROL_SHADER);
		glAttachShader(program, objects[SHADER_TESSCONTROL]);
	}
	if (!eFile.empty()) {
		objects[SHADER_TESSEVAL] = GenerateShader(eFile, GL_TESS_EVALUATION_SHADER);
		glAttachShader(program, objects[SHADER_TESSEVAL]);
	}
